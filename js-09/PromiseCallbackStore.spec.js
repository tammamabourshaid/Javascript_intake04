const { BasketWithCallbackStore } = require("./BasketWithCallbackStore");
const { BasketWithPromiseStore } = require("./BasketWithPromiseStore.js");
...
`, () => {
  ...
  class CallbackStore {
    getProductForName(productName, callback) {
      setTimeout(() => callback(getProductForName(productName)), 100);
    }
  }

  class PromiseStore {
    getProductForName(productName) {
      return new Promise(resolve =>
        setTimeout(() => resolve(getProductForName(productName)), 100)
      );
    }
  }

  it("should work with the BasketWithPromiseStore", done => {
    const basket = new BasketWithPromiseStore(new PromiseStore());
    basket.put("bread", 3, () => {
